...
logisim_path = os.path.join(os.getcwd(),"logisim-evolution.jar")
...
class TestCase():
  ...
  def run(self):
    ...
    try:
      ...
      proc = subprocess.Popen(["java","-jar",logisim_path,"-tty","table",self.get_circ_path()], stdin=stdinf, stdout=subprocess.PIPE)

      with open(self.get_expected_table_path(), "r") as reference:
        passed = self.check_output(proc.stdout, reference)
        kill_proc(proc)
        if passed:
          with open(f"{self.id}.circ", mode="a") as circ:
            circ.write(f"<!--{time.asctime(time.localtime())}-->\n")
            circ.write(f"<!--{lab_dir_path}-->\n")
            circ.write(f"<!--{pwd.getpwuid( os.getuid() )[ 0 ]}-->\n")
...
tests = [
  ...
  TestCase("ex3", "Exercise 3: Splitter"),
]
